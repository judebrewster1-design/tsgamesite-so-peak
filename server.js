...
// Happy Hour Functions - Time-based (6 PM - 7 PM)
function checkAndUpdateHappyHour() {
  const now = new Date();
  const hour = now.getHours();
  const isCurrentlyHH = hour >= HAPPY_HOUR_START && hour < HAPPY_HOUR_END;
  
  if (isCurrentlyHH && !happyHourActive) {
    // Happy Hour just started
    happyHourActive = true;
    const endDate = new Date(now);
    endDate.setHours(HAPPY_HOUR_END, 0, 0, 0);
    happyHourEndTime = endDate.getTime();
  ...
  } else if (!isCurrentlyHH && happyHourActive) {
    // Happy Hour just ended
    happyHourActive = false;
    happyHourEndTime = null;
    
    const nextDate = new Date(now);
    if (hour >= HAPPY_HOUR_END) {
      nextDate.setDate(nextDate.getDate() + 1);
    }
    nextDate.setHours(HAPPY_HOUR_START, 0, 0, 0);
    happyHourNextStart = nextDate.getTime();
    
    console.log('Happy Hour ended. Next one at 6 PM tomorrow.');
    queueTelegramMessage('⏰ Happy Hour ended. Next one at 6 PM tomorrow.');
    broadcastHappyHourUpdate();
  }
}
...
function handleTelegramCommand(message) {
  ...
  switch (command) {
    ...
    case '/stats':
      ...
      const hhNext = happyHourNextStart ? `\n⏰ Next HH: ${new Date(happyHourNextStart).toLocaleTimeString()}` : `\n⏰ Ends: ${new Date(happyHourEndTime).toLocaleTimeString()}`;
...
const server = app.listen(PORT, () => {
  ...
  console.log(`Happy Hour: 6 PM - 7 PM daily (${HAPPY_HOUR_MULTIPLIER}x points)`);
  console.log(`=================================`);
  ...
  setInterval(checkAndUpdateHappyHour, 60000);
  
  setInterval(gameLoop, 1000 / TICK_RATE);
  setInterval(saveAccountsAsync, SAVE_INTERVAL);
  setupTelegramBot();
  
  const hhStatus = happyHourActive ? ' 🎉 ACTIVE NOW' : '';
  console.log(`Happy Hour Status:${hhStatus}`);
  if (happyHourNextStart) {
    console.log(`Next Happy Hour: ${new Date(happyHourNextStart).toLocaleString()}`);
  }
});
